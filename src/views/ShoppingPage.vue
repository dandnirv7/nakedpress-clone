...
const dailyProduct330MlStore = useDailyProduct330MlStore();
const merchandisesStore = useMerchandisesStore();
...
const { category: dailyKidsCategory } = toRefs(
  dailyKidsProductStore.dailyKidsProduct
);
const { category: detoxProductCategory } = toRefs(
  detoxProductStore.detoxProduct
);
const { category: oneLiterProductCategory } = toRefs(
  oneLiterProductStore.oneLiterProduct
);
const { category: dailyProduct330Category } = toRefs(
  dailyProduct330MlStore.dailyProduct330
);
const { category: merchandisesStoreCategory } = toRefs(
  merchandisesStore.merchandise
);
...
export default {
  ...
  methods: {
    toggleDropdown() {
      return (this.isActiveDropdown = !this.isActiveDropdown);
    },
  },
};
</script>
...
<template>
  <div>
    <nav
      class="flex flex-row items-center justify-between px-5 py-5 bg-white z-99"
      id="sticky-header"
    >
      <li
        class="list-none"
